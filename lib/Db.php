...
class Db {
    ...
    function count($table, $where) {
        if ($where != "")
            $sel = "SELECT COUNT(*) AS cnt FROM " . $table . " WHERE " . $where;
        else
            $sel = "SELECT COUNT(*) AS cnt FROM " . $table;
    ...
    function numberToWords($number) {
        ...
        if (($number >= 0 && (int) $number < 0) || (int) $number < 0 - PHP_INT_MAX) {
            // overflow
            trigger_error(
                    'Out of Range Numbers: This function only accepts numbers between - ' . PHP_INT_MAX . ' and ' . PHP_INT_MAX, E_USER_WARNING
            );
            return false;
        }
    ...
    function getFieldRecord($table, $field, $where, $ordby) {
        if ($where != "")
            $sel = "SELECT " . $field . " FROM " . $table . " WHERE " . $where;
        else
            $sel = "SELECT " . $field . " FROM " . $table;
        if ($ordby != "")
            $sel = $sel . " ORDER BY " . $ordby;
    ...
    function add($table, $fields, $values) {
        ...
        for ($i = 0; $i < count($fields); $i++) {
            if (substr($values{$fields[$i]}, 0, 1) == "!")
                $va = $va . substr($values{$fields[$i]}, 1, strlen($values{$fields[$i]}) - 1);
            else
                $va = $va . "'" . $values{$fields[$i]} . "'";
            if ($i < (count($fields) - 1))
                $va = $va . ",";
        }
    ...
    function update($table, $fields, $values, $where) {
        ...
        for ($i = 0; $i < count($fields); $i++) {
            if (substr($values{$fields[$i]}, 0, 1) == "!")
                $va = $va . $fields[$i] . " = " . substr($values{$fields[$i]}, 1, strlen($values{$fields[$i]}) - 1);
            else
                $va = $va . $fields[$i] . " = '" . $values{$fields[$i]} . "'";
            if ($i < (count($fields) - 1))
                $va = $va . ",";
        }
...
//end getfrec
    ...
    function getmax($table, $field, $where, $ordby, $limit) {
        if ($where != "")
            $sel = "SELECT max(" . $field . ") FROM " . $table . " WHERE " . $where;
        else
            $sel = "SELECT max(" . $field . ") FROM " . $table;
        if ($ordby != "")
            $sel = $sel . " ORDER BY " . $ordby;
        if ($limit != "")
            $sel = $sel . " LIMIT " . $limit;
...
//end getmax
    ...
    function getmin($table, $field, $where, $ordby, $limit) {
        if ($where != "")
            $sel = "SELECT min(" . $field . ") FROM " . $table . " WHERE " . $where;
        else
            $sel = "SELECT min(" . $field . ") FROM " . $table;
        if ($ordby != "")
            $sel = $sel . " ORDER BY " . $ordby;
        if ($limit != "")
            $sel = $sel . " LIMIT " . $limit;
...
//end getmax
    ...
    function getfarray($table, $se, $where, $ordby, $limit) {
        if ($where != "")
            $sel = "SELECT " . $se . " FROM " . $table . " WHERE " . $where;
        else
            $sel = "SELECT " . $se . " FROM " . $table;
        if ($ordby != "")
            $sel = $sel . " ORDER BY " . $ordby;
        if ($limit != "")
            $sel = $sel . " LIMIT " . $limit;
    ...
    function getarrayunique($table, $where, $ordby, $limit) {
        ...
        if ($where != "")
            ...
            $sel = "SELECT DISTINCT FROM " . $table;
        if ($ordby != "")
            $sel = $sel . " ORDER BY " . $ordby;
        if ($limit != "")
            $sel = $sel . " LIMIT " . $limit;
    ...
    function getfarrayunique($table, $field, $where, $ordby, $limit) {
        ...
        $i = 0;
        if ($where != "")
            $sel = "SELECT DISTINCT " . $field . " FROM " . $table . " WHERE " . $where;
        else
            $sel = "SELECT  DISTINCT " . $field . " FROM " . $table;
        if ($ordby != "")
            $sel = $sel . " ORDER BY " . $ordby;
        if ($limit != "")
            $sel = $sel . " LIMIT " . $limit;
