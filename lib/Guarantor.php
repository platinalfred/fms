...
class Guarantor extends Db {
	...
	public function getLoanGuarantors($loanAccountId){
		...
		$members_sql = "(SELECT `member`.`id`, `phone`, CONCAT(`firstname`, ' ', `lastname`, ' ', `othername`) `memberNames` FROM `member` JOIN `person` ON `member`.`memberId` = `person`.`id`) `members`";
		
		$savings_sql = "(SELECT SUM(`amount`) savings, `memberId` FROM `account_transaction` WHERE `transactionType`=1 GROUP BY `memberId`) `client_savings`";
		
		$shares_sql = "(SELECT SUM(`amount`) `shares`, `memberId` FROM `shares` GROUP BY `memberId`) `client_shares` ";
		...
		$table = self::$table_name." JOIN ".$members_sql." ON `members`.`id` = ".(self::$table_name).".`memberId` LEFT JOIN ".$savings_sql." ON `client_savings`.`memberId` = ".(self::$table_name).".`memberId` JOIN ".$shares_sql." ON ".(self::$table_name).".`memberId` = `client_shares`.`memberId` JOIN ".$loan_balances_sql." ON ".(self::$table_name).".`memberId` = loan_balances`.`memberId`";
